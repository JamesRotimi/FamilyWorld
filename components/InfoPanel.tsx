 ...
 * structured rows, hairline rules. Designed to feel integrated with
 * the scene to the right, not a floating card.
 *
 * Token-bound: surface, line, panel shadow, padding all come from
 * /design-tokens.json via Tailwind utilities.
 */
export default function InfoPanel({ items, recentItem, onSelect, onScan }: Props) {
  ...
  return (
    <aside className="relative flex w-full flex-col gap-6 bg-surface p-6 lg:w-[28%] lg:flex-none lg:border-r lg:border-line/70 lg:px-7 lg:py-8 xl:w-[24%]">
      <header className="flex flex-col gap-2">
        <span className="text-ui-micro font-semibold uppercase text-ink-soft">
          FamilyWorld · v1
        </span>
        <h2 className="font-display text-ui-heading font-semibold text-ink">
          General Entry into your FamilyWorld Home
        </h2>
        <p className="text-ui-small font-medium leading-relaxed text-ink-soft">
      ...
      <Section heading="Reminders" tone="alert">
        {bike?.reminder && (
          <div className="flex items-start gap-2.5 rounded-md border border-primary/30 bg-primary/5 px-2.5 py-2">
            <span aria-hidden className="mt-1 h-1.5 w-1.5 flex-none rounded-full bg-primary-notify" />
            <div>
              <div className="text-[12.5px] font-semibold tracking-[-0.005em] text-ink">
                {bike.reminder.text}
              </div>
      ...
      <Section heading="Best next action">
        <button
          type="button"
          onClick={onScan}
          className="flex w-full items-center justify-between gap-3 rounded-md border border-line bg-surface-frame px-3 py-2.5 text-left transition hover:border-primary-soft hover:bg-white"
        >
          <div className="min-w-0">
            <div className="text-ui-label font-semibold text-ink">
              Scan a receipt
            </div>
      ...
      <p className="mt-auto pt-4 text-ui-micro font-medium uppercase text-ink-mute">
  ...
  return (
    <section className="border-t border-line/60 pt-4">
      <div className="mb-2 flex items-center gap-2">
        <h3 className="text-ui-micro font-bold uppercase text-ink-soft">
          {heading}
        </h3>
        {tone === "alert" && (
          <span className="h-1.5 w-1.5 rounded-full bg-primary-notify" aria-hidden />
        )}
      </div>
...
function KeyVal({ k, v, muted = false }: { k: string; v: string; muted?: boolean }) {
  return (
    <div className="flex items-baseline justify-between gap-3 border-b border-dotted border-line/60 pb-1 last:border-b-0">
      <span className="text-[11.5px] font-medium text-ink-soft">{k}</span>
      <span
        className={`text-ui-small tabular-nums ${muted ? "font-medium text-ink-mute" : "font-semibold text-ink"}`}
      >
        {v}
      </span>
    </div>
...
function ItemRow({ item, onSelect }: { item: FamilyItem; onSelect: (id: string) => void }) {
  return (
    ...
    >
      <span className="text-[11px] font-bold uppercase tracking-[0.12em] text-primary-notify">●</span>
      <span className="flex min-w-0 flex-1 flex-col leading-tight">
        <span className="text-ui-label font-semibold text-ink">{item.name}</span>
        <span className="truncate text-[10.5px] font-medium text-ink-soft">
          {item.reminder ? `Next · ${item.reminder.text}` : item.type}
        </span>
      </span>
      <span className="rounded-full bg-primary/15 px-1.5 py-px text-[9px] font-bold uppercase tracking-[0.1em] text-primary-notify">
        live
      </span>
    </button>
  );
